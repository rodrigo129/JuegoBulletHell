...
class DLine2(Disparo):
    ...
    YVelocidad=None
    XYOobjetivo=None
    def __init__(self,XYA,XYO,Vel,RGB):#self,CoordenadaX,CoordenadaY,Lista CoordenadaObjetivo,Velicidad,Color##proximamente indice sprite
        self.X=XYA[0]
        self.Y=XYA[1]
        self.Tiempo=0
        self.XYObjetivo=XYO
        ...
        if self.X==self.XYObjetivo[0]:#angulo 90 o 270
            ...
            if self.Y>self.XYObjetivo[1]:
                if self.X<self.XYObjetivo[0]:
                    ...
                    self.Omega=math.degrees(math.atan(-(self.Y-self.XYObjetivo[1])/(self.X-self.XYObjetivo[0])))#+/-
                else:
                    #print("cuadrante 2","arma")
                    self.Omega=math.degrees(math.atan(-(self.Y-self.XYObjetivo[1])/(self.X-self.XYObjetivo[0])))+180
            else:
                if self.X<self.XYObjetivo[0]:
                    ...
                    self.Omega=abs(math.degrees(math.atan(-(self.Y-self.XYObjetivo[1])/(self.X-self.XYObjetivo[0])))+360)
